...
class CXR_Retrieval_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.is_train:
            ...
            print("pair_pos", pair_pos.shape)
            print("pair_neg", pair_neg.shape)
            input("SOTP!!1")
...
class CXR_mixer(nn.Module):
    def __init__(self, config, args):
        ...
        self.cxrmixer = MLPMixer(image_size = 256, channels = 3, patch_size = 16, dim = 512, depth = 12, num_classes = 2)

    def forward(self, cls_tok, input_txt, attn_mask):
        _, cls, _ = self.cxrmixer(cls_tok, input_txt, attn_mask)
        result = self.itm(cls)
        return result


def main(args):
    wandb.init(config=args, project='fianl_project', entity='optimal_transport')
    ...
    if args.CXRBERT:
        ...
        if args.do_train:
            model = MLPMixer(config, args).to(args.device)
        elif args.do_test:
            ...
            model = MLPMixer.from_pretrained(args.load_pretrained_model, state_dict=model_state_dict, config=config, args=args).to(args.device)
    ...
    if args.do_train:
        ...
        print("Load Train dataset", train_dataset)
        input("STOP!!!")
